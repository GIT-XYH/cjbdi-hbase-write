 ...
 * @Description: 使用 java 将图片生成SequenceFile 并保存到 HBase
 */
public class SequenceFileTest {
    //HDFS路径
    static String inpath = "/tmp/xyh_test/图片测试.png";
    static String outpath = "/tmp/xyh_test/out";
...
//        htable = new HTable(hbaseConf,"picHbase");
        Table htable = conn.getTable(TableName.valueOf("ns_ws:t_ws_test"));
...
//            String rowKey = Integer.valueOf(tmp[0])-1+"_"+Integer.valueOf(tmp[1])/2+"_"+Integer.valueOf(tmp[2])/2;
            ...
            put.addColumn("ws_xx".getBytes(), "picture".getBytes() , val.getBytes());
            htable.put(put);
