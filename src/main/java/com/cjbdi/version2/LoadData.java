...
public class LoadData {
    ...
    public static void main(String[] args) throws Exception {
        ...
        //获取table对象
        TableName tableName = TableName.valueOf("ns_xyh:t_pic");
        Table table = connection.getTable(tableName);
            HBaseBulkLoad.bulkLoad(args);
            //构建LoadIncrementalHFiles加载HFile文件
            LoadIncrementalHFiles load = new LoadIncrementalHFiles(configuration);
            load.doBulkLoad(new Path("hdfs://bd-01:8020/tmp/xyh/pic_hfile"), admin, table, connection.getRegionLocator(tableName));
            try {
                FileSystem fs = FileSystem.get(new URI("hdfs://bd-01:8020"), configuration);
                fs.delete(new Path("/tmp/xyh/doc_pic"));
                fs.close();
            } catch (IOException e) {
                e.printStackTrace();
            }
